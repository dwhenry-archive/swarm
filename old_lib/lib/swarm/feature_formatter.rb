module Swarm
  class FeatureFormatter
    ...
    def step_name(keyword, step_match, status, source_indent, background)
      ...
      when :undefined
        test_result_handler.test_undefined "#{step_match.format_args(lambda{|param| "*#{param}*"})} UNDEFINED"
      when :pending
    ...
    def after_table_row(table_row)
      if table_row.exception
        test_result_handler.test_failed(table_row.exception.backtrace.last, "#{table_row.exception.message} (#{table_row.exception.class})\n#{table_row.exception.backtrace.join("\n")}")
      end
    end
    ...
    def report_failures_and_clear
      @last_failed_test.each { |exception| test_result_handler.test_failed(exception.backtrace.last, "#{exception.message}\n#{exception.backtrace.join("\n")}") }
